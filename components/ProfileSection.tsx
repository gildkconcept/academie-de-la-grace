...
export const ProfileSection = ({ user, onClose }: ProfileSectionProps) => {
  ...
  const handleProfileUpdate = async () => {
    ...
    try {
      const body: any = { name: profileData.name, username: profileData.username, email: profileData.email }
          ...
          <div className="flex flex-col sm:flex-row items-center gap-4">
            ...
            <div className="flex-1 text-center sm:text-left">
              ...
              <div className="flex gap-2 mt-3 justify-center sm:justify-start">
                ...
                {profileData.profileImageUrl && (
                  <button onClick={handlePhotoDelete} disabled={uploadingPhoto} className="inline-flex items-center gap-1 px-3 py-1.5 bg-red-500/20 text-red-300 rounded-lg text-xs hover:bg-red-500/30 transition-colors">
              ...
              {user?.role === 'student' && (
                <>
                  <input type="tel" value={profileData.phone} onChange={(e) => setProfileData({ ...profileData, phone: e.target.value })} placeholder="Téléphone"
                    className="w-full px-4 py-2.5 bg-white/90 border border-white/30 rounded-lg text-gray-900 placeholder-gray-500 text-sm focus:outline-none focus:border-indigo-400" />
                  <select value={profileData.baptized ? 'true' : 'false'} onChange={(e) => setProfileData({ ...profileData, baptized: e.target.value === 'true' })}
